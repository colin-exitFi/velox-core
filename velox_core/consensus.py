...
PROMPT_TEMPLATE = """You are a trading agent inside Velox Core, an autonomous paper trading bot.
Your job: score each of the {n} stocks below and return a structured action for each.

CURRENT TIME: {now} ET ({session_label})
STARTING EQUITY: ${equity:,.0f}
...
For EACH symbol return one of:
  BUY    — initiate a long if no position; clear conviction the price moves up in 30-90 min
  SHORT  — initiate a short if no position; clear conviction the price moves down in 30-90 min
  HOLD   — no action; we already hold OR no clear setup
...
def _token_budget_for_universe_size(n: int) -> int:
    """Compute output token budget that scales with the universe size.
    Each ticker vote is ~60-100 tokens of JSON. Headroom for reasoning.
    Minimum 4000, clamp at 16000.
    """
    # 120 tokens per vote entry × n + 400 scaffolding buffer
...
async def _call_claude(prompt: str, token_budget: int = 4000) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            ...
            pass
        logger.error(
            f"Claude call failed: HTTP {e.response.status_code if e.response else '?'} "
            f"body={body!r}"
        )
        return None
...
async def _call_gpt(prompt: str, token_budget: int = 4000) -> Optional[Dict]:
    ...
    try:
        ...
        try:
            ...
            pass
        logger.error(
            f"GPT call failed: HTTP {e.response.status_code if e.response else '?'} "
            f"body={body!r}"
        )
        return None
    except Exception as e:
...
async def run_consensus(
    ...
    # Scale output token budget with universe size so GPT's json_object response
    # doesn't get silently truncated mid-array. 120 tokens per ticker × N + buffer.
